...
#pragma pack(1)
typedef struct {
    u8 zeros[3];        /* 3 bytes 0 */
    u8 flux[4];         /* 4 bytes */
    ipv4_t cip;         /* cient IP */
} drcom_nrml_02_t;

typedef struct {
    u8 usrlen;          /* 本校长度是12,暂且定位12固定的 */
    u8 cmac[ETH_ALEN];
    ipv4_t cip;
    u8 fixed[4];
    u8 flux[4];         /* 回应服务器的flux */
    u8 verify[4];       /* 校验值 */
    u8 zeros[4];        /* 4 bytes 0 */
    u8 user[12];        /* user和hostname总长：28 bytes, 这里为了方便先固定为12 */
    u8 host[16];        /* host: 主机名 */
    u8 ign[16];         /* 后面貌似已经没有影响了长度是nrml_len-上述所占长度*/
} drcom_nrml_03_t;
